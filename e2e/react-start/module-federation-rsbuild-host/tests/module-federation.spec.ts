...
test('serves federation manifest and stats endpoints as JSON', async ({ page }) => {
  ...
  for (const [
    ...
    expect(reactDom?.assets?.js?.async ?? []).toEqual([])
    expect(react?.assets?.css?.sync ?? []).toEqual([])
    expect(reactDom?.assets?.css?.sync ?? []).toEqual([])
    expect(react?.assets?.css?.async ?? []).toEqual([])
    expect(reactDom?.assets?.css?.async ?? []).toEqual([])
    ...
    for (const [exposeName, exposePath] of Object.entries(expectedExposePaths)) {
      ...
      expect(exposeSyncAssets.length).toBeGreaterThan(0)
      assertRelativeJsAssetPaths(exposeSyncAssets)
      expect(expose?.assets?.js?.async ?? []).toEqual([])
      expect(expose?.assets?.css?.sync ?? []).toEqual([])
      expect(expose?.assets?.css?.async ?? []).toEqual([])
      if (isStatsEndpoint) {
